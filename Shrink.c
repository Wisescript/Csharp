...
int main()
{
int i, fd = open("/storage/emulated/0/Edited/L.html", O_RDONLY);
...
#define C "/system/bin/curl -H \"User-Agent:Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36(KHTML, like Gecko) Chrome/75.0.3770.100 Safari/537.36\" "
  
#define url "-i -s https://is.gd/" 
#define geturl(URL) popen(URL, "r" )
#define prefix "Location: http://ok.com/?"

char *after(char *str, char *src) {
    int b = (*src) ? strlen(src) : 0;
    char *s=strstr(str, src);
    if (s != 0)
    return s+b;
    return 0;
}
...
{
    char *id="housemusic";
    char *curl=malloc(1120);
    char *pipe=malloc(112000);

	    sprintf(curl, "%s %s%s",C,url, id);
    FILE* p=geturl(curl) ;
    while (fgets(pipe, 112000, p) != NULL) { 
      printf(after(pipe, prefix)) ;
    }
